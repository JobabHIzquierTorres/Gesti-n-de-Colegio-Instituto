...
@section('title', 'Modificar Calificación')
@section('content')
                    ...
                    <div class="col-lg-5">
                        <div class="card shadow-lg border-0 rounded-lg mt-5">
                            <div class="card-header">
                                <h3 class="text-center font-weight-light my-4">Modificar Calificación</h3>
                            </div>
                            <div class="card-body">
                                <form id="form-calificacion" method="POST" action="{{ route('calificaciones.update', $calificacion->ID_Calificacion) }}">
                                    @csrf
                                    @method('PUT')
                                        ...
                                        <select class="form-select" id="estudiante" name="ID_Estudiante" required>
                                            ...
                                            @foreach($estudiantes as $estudiante)
                                                <option value="{{ $estudiante->ID_Estudiante }}" 
                                                    @if($estudiante->ID_Estudiante == $calificacion->ID_Estudiante) selected @endif>
                                                    {{ $estudiante->Nombre }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select class="form-select" id="curso" name="ID_Curso" required>
                                            ...
                                            @foreach($cursos as $curso)
                                                <option value="{{ $curso->ID_Curso }}" 
                                                    @if($curso->ID_Curso == $calificacion->ID_Curso) selected @endif>
                                                    {{ $curso->Nombre }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-floating mb-3">
                                        <input class="form-control" id="nota" name="Nota" 
                                               ...
                                               placeholder="Ej: 7.5" 
                                               value="{{ number_format($calificacion->Nota / 10, 1) }}" required>
...
{{-- Script para convertir nota 0–10 a 0–100 antes de enviar --}}
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        form.addEventListener('submit', function (e) {
            ...
            if (!isNaN(nota)) {
                notaInput.value = Math.round(nota * 10);
            }
        });
    });
</script>
